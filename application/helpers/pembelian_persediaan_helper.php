...
function persediaan_recalculate_sum_penjualan_for_row($CI, $tgl_awal, $tgl_akhir, $row)
{
	...
	$list = $CI->db->query(
		"SELECT `uuid_barang`, `nama_barang`, `satuan`, `harga_satuan`, `jumlah`
		FROM `tbl_penjualan`
		WHERE DATE(`tgl_jual`) >= ? AND DATE(`tgl_jual`) <= ?",
		array($tgl_awal, $tgl_akhir)
	)->result();
...
/**
 * Record bulan sumber layak di-copy: sa atau beli atau total_10 ada nilai; skip jika sa=0 dan beli=0.
 */
function persediaan_generate_recalculate_sumber_layak_generate($row)
{
	...
	$beli = persediaan_parse_angka(is_array($row) ? (isset($row['beli']) ? $row['beli'] : 0) : (isset($row->beli) ? $row->beli : 0));
...
/**
 * SQL tambahan: baris sumber layak generate (bukan sa=0 dan beli=0).
 */
...
function persediaan_generate_recalculate_upsert_from_sumber($CI, $ctx, $row_sumber, &$next_id, &$map)
{
	...
	if (!persediaan_generate_recalculate_sumber_layak_generate($row_sumber)) {
		...
		return array(
			...
			'keterangan' => 'Lewati: sa=0 dan beli=0 di bulan sumber — tidak di-copy ke bulan target',
		);
	}
...
/**
 * Pastikan penjualan/total_10/tuj/kolom unit konsisten: penjualan <= sa+beli, total_10 = sa+beli-penjualan.
 */
...
function persediaan_generate_recalculate_reapply_penjualan_bulan($CI, $ctx)
{
	...
	$flushed = persediaan_recalculate_flush_accum_to_db($CI, $accum);
	...
	return array(
		...
		'flushed' => $flushed,
		'reconcile' => $reconcile,
		'with_penjualan' => count(array_filter($accum, function ($a) {
			return (int) $a['penjualan'] > 0;
		})),
	);
}
...
function persediaan_generate_recalculate_proses_penjualan_row($CI, $ctx, $queue_item, &$map, &$accum)
{
	...
	$total_10_baru = max(0, $gross - $penjualan_baru);
	$nilai = (int) floor($total_10_baru * persediaan_parse_angka(isset($existing->hpp) ? $existing->hpp : 0));
	$keterangan = 'penjualan+=' . $allocated . ' (maks stok sa+beli=' . $gross . ') → total_10/sisa=' . $total_10_baru;
	if ((int) $alloc['remaining'] > 0) {
...
function persediaan_generate_recalculate_finish_all($CI, $bulan, $ctx, $count_sumber, &$state, $state_key, $batch_items)
{
	...
	return array_merge(array(
		...
		'pesan' => 'Selesai: generate dari bulan sumber (sa/beli ada nilai), pembelian (match spop), penjualan dari tbl_penjualan, bahan produksi dari sys_unit_produk_bahan, pecah satuan dari tbl_pembelian_pecah_satuan.'
			. $pesan_penj
			. $pesan_prod
			. $pesan_pecah
			. $pesan_nilai
...
function persediaan_generate_recalculate_batch($CI, $bulan, $offset, $limit, $start = false)
{
	...
	if ($state['phase'] === 'generate') {
		...
		return array(
			...
			'items_persediaan' => $items_persediaan,
			'stats' => $state['stats'],
			'pesan' => 'Generate fase 1: ' . $offset_selesai . ' / ' . $count_sumber . ' record sumber (sa atau total_10 > 0)'
				. (isset($ctx['count_sumber_skip_total10']) && (int) $ctx['count_sumber_skip_total10'] > 0
					? ', lewati ' . (int) $ctx['count_sumber_skip_total10'] . ' record sa=0 & total_10=0' : ''),
		);
	}
	...
	if ($state['phase'] === 'penjualan') {
		...
		if ($done) {
			if (!empty($state['penjualan_accum'])) {
				persediaan_recalculate_flush_accum_to_db($CI, $state['penjualan_accum']);
			}
...
function persediaan_compare_excel_all_group_definitions()
{
	return array(
		...
		array('title' => 'DATA PEMBELIAN', 'col_start' => 20, 'col_end' => 24),
		array('title' => 'DATA PENJUALAN', 'col_start' => 26, 'col_end' => 29),
		array('title' => 'DATA PRODUKSI', 'col_start' => 31, 'col_end' => 35),
		array('title' => 'DATA PECAH SATUAN', 'col_start' => 37, 'col_end' => 41),
	);
}
...
function persediaan_compare_excel_all_headers()
{
	return array(
		...
		'Uraian', 'Satuan', 'Harga Satuan', 'SPOP', 'Jumlah', '',
		'Nama Barang', 'Satuan', 'Harga Satuan', 'Jumlah', '',
		'Nama Barang', 'Satuan', 'Harga Satuan', 'SPOP', 'Jumlah', '',
		'Nama Barang', 'Satuan', 'Harga Satuan', 'SPOP', 'Jumlah',
	);
}
...
function persediaan_compare_excel_all_row_sort_key($row)
{
	...
	foreach ($sections as $section) {
		...
		$nama = isset($row[$starts[$section]]) ? trim((string) $row[$starts[$section]]) : '';
		if ($nama !== '') {
			return persediaan_recalculate_normalize_nama($nama);
		}
	}
...
function persediaan_compare_excel_all_fill_manual(&$row, $manual_row, $map, $extra_cols = array())
{
	...
	$row[$s + 2] = persediaan_compare_row_get($manual_row, $map['hpp']);
	$row[$s + 3] = persediaan_compare_row_get($manual_row, $map['spop']);
	...
	$row[$s + 4] = ($sa_col !== null && $sa_col !== '')
		? persediaan_compare_row_get($manual_row, $sa_col) : '';
	$row[$s + 5] = ($beli_col !== null && $beli_col !== '')
		? persediaan_compare_row_get($manual_row, $beli_col) : '';
	$row[$s + 6] = ($tuj_col !== null && $tuj_col !== '')
		? persediaan_compare_row_get($manual_row, $tuj_col) : '';
	$row[$s + 7] = ($col_10 !== null && $col_10 !== '')
		? persediaan_compare_row_get($manual_row, $col_10) : '';
	$row[$s + 8] = ($nilai_col !== null && $nilai_col !== '')
		? persediaan_compare_row_get($manual_row, $nilai_col) : '';
}
...
function persediaan_compare_excel_all_fill_persediaan(&$row, $prow)
{
	...
	$row[$s + 2] = isset($prow->hpp) ? $prow->hpp : '';
	$row[$s + 3] = isset($prow->spop) ? $prow->spop : '';
	$row[$s + 4] = isset($prow->sa) ? $prow->sa : '';
	$row[$s + 5] = isset($prow->beli) ? $prow->beli : '';
	$row[$s + 6] = isset($prow->tuj) ? $prow->tuj : '';
	$row[$s + 7] = isset($prow->total_10) ? $prow->total_10 : '';
	$row[$s + 8] = $nilai_persediaan;
}
...
function persediaan_compare_excel_all_fill_pembelian(&$row, $r)
{
	$starts = persediaan_compare_excel_all_col_starts();
	$s = $starts['pembelian'];
	$row[$s] = isset($r->uraian) ? $r->uraian : '';
	$row[$s + 1] = isset($r->satuan) ? $r->satuan : '';
	$row[$s + 2] = isset($r->harga_satuan) ? $r->harga_satuan : '';
	$row[$s + 3] = isset($r->spop) ? $r->spop : '';
	$row[$s + 4] = isset($r->jumlah) ? $r->jumlah : '';
}
...
function persediaan_compare_excel_all_fill_penjualan(&$row, $r)
{
	$starts = persediaan_compare_excel_all_col_starts();
	$s = $starts['penjualan'];
	$row[$s] = isset($r->nama_barang) ? $r->nama_barang : '';
	$row[$s + 1] = isset($r->satuan) ? $r->satuan : '';
	$row[$s + 2] = isset($r->harga_satuan) ? $r->harga_satuan : '';
	$row[$s + 3] = isset($r->jumlah) ? $r->jumlah : '';
}
...
function persediaan_compare_excel_all_fill_produksi(&$row, $r, $spop = '')
{
	$starts = persediaan_compare_excel_all_col_starts();
	$s = $starts['produksi'];
	$row[$s] = isset($r->nama_barang_bahan) ? $r->nama_barang_bahan : '';
	$row[$s + 1] = isset($r->satuan_bahan) ? $r->satuan_bahan : '';
	$row[$s + 2] = isset($r->harga_satuan_bahan) ? $r->harga_satuan_bahan : '';
	$row[$s + 3] = $spop;
	$row[$s + 4] = isset($r->jumlah_bahan) ? $r->jumlah_bahan : '';
}

function persediaan_compare_excel_all_fill_pecah(&$row, $nama, $satuan, $harga, $spop, $jumlah)
{
	$starts = persediaan_compare_excel_all_col_starts();
	$s = $starts['pecah'];
	$row[$s] = $nama;
