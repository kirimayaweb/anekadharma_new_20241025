...
class Tbl_penjualan_jasa extends CI_Controller
{
	function __construct()
	{
		parent::__construct();
		is_login();
		$this->load->model(array('Tbl_penjualan_jasa_model', 'Tbl_pembelian_model', 'Sys_konsumen_model', 'Sys_unit_model', 'Sys_nama_barang_model', 'Persediaan_model', 'Buku_besar_model', 'Tbl_penjualan_cetak_pembayaran_model'));
	...
	{
		$this->session->set_userdata('filter_tbl_penjualan_jasa_date_awal', $date_awal);
		$this->session->set_userdata('filter_tbl_penjualan_jasa_date_akhir', $date_akhir);
		if ($tgl_awal_display !== null && $tgl_akhir_display !== null) {
			$this->session->set_userdata('filter_tbl_penjualan_jasa_tgl_awal_display', $tgl_awal_display);
			$this->session->set_userdata('filter_tbl_penjualan_jasa_tgl_akhir_display', $tgl_akhir_display);
		}
		if ($rows !== null) {
			$this->session->set_userdata('filter_tbl_penjualan_jasa_ids', $this->_collect_row_ids($rows));
		}
	}
	...
	{
		$col = ($alias !== '') ? $alias . '.barang_jasa' : 'barang_jasa';
		return " AND {$col} = 'jasa' ";
	}
	...
	private function _resolve_penjualan_filter_dates()
	{
		...
		$awal = $this->session->userdata('filter_tbl_penjualan_jasa_date_awal');
		$akhir = $this->session->userdata('filter_tbl_penjualan_jasa_date_akhir');
		$disp_awal = $this->session->userdata('filter_tbl_penjualan_jasa_tgl_awal_display');
		$disp_akhir = $this->session->userdata('filter_tbl_penjualan_jasa_tgl_akhir_display');
		if ($awal && $akhir) {
			if (!$disp_awal) {
	...
	public function RekapPenjualanPerBarang()
	{
		...
		// 	-- left join tbl_penjualan_jasa ON persediaan.uuid_barang = tbl_penjualan.uuid_barang  
		// 	Full Outer Join tbl_penjualan_jasa ON persediaan.uuid_barang = tbl_penjualan.uuid_barang  
		...
		$sql_persediaan = "SELECT persediaan.id as id,
							...
							FROM persediaan
							right JOIN tbl_penjualan ON persediaan.id = tbl_penjualan.id_persediaan_barang AND tbl_penjualan.barang_jasa = 'jasa'
		...
		$data = array(
			...
			// 'action_cari_konsumen' => site_url('tbl_penjualan_jasa/rekap'),
		...
		// $this->load->view('anekadharma/tbl_penjualan_jasa/tbl_penjualan_list', $data);		
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_penjualan_jasa/adminlte310_tbl_penjualan_list_rekap', $data);
	}
	...
	public function RekapPenjualanPerKonsumen()
	{
		...
		-- 						persediaan.spop as spop, 
		 						...
		 						tbl_penjualan.harga_satuan as harga_satuan_penjualan,
		 						tbl_penjualan.uuid_persediaan as uuid_persediaan_penjualan
							FROM tbl_penjualan WHERE barang_jasa = 'jasa' -- right JOIN  tbl_penjualan_jasa ON persediaan.id= tbl_penjualan.id_persediaan_barang
		...
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_penjualan_jasa/adminlte310_tbl_penjualan_list_rekap_per_konsumen', $data);
	}
	...
	public function RekapData($field_rekap = null)
	{
		...
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_penjualan_jasa/adminlte310_tbl_penjualan_list_rekap_data', $data);
	}
	...
	public function read($id)
	{
		...
		if ($row) {
			...
			$this->load->view('anekadharma/tbl_penjualan_jasa/tbl_penjualan_read', $data);
		} else {
	...
	public function create()
	{
		...
		$data = array(
			'button' => 'Simpan',
			'action' => site_url('tbl_penjualan_jasa/create_action'),
			'id' => set_value('id'),
		...
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_penjualan_jasa/adminlte310_tbl_penjualan_form', $data);
	}
	...
	public function create_action_inisiasi($id_proses = null)
	{
		...
		try {
			$Data_stock = penjualan_get_stock_persediaan_jasa_rows($this, $tgl_jual_post, $Get_UUID_unit);
		} catch (Exception $e) {
		...
		$data = array(
			...
			'action_ubah_detail_nomor_kirim' => site_url('tbl_penjualan_jasa/action_ubah_detail_nomor_kirim/' . $this->input->post('nmrkirim', TRUE) . '/new'),
		);
		// 		print_r($data);
		// die;
		// $this->load->view('anekadharma/tbl_penjualan_jasa/tbl_penjualan_form', $data);
		$data['jumlah_jasa_penjualan'] = 0;
	...
	public function list_persediaan_penjualan_jasa_ajax()
	{
		try {
			...
			$Data_stock = penjualan_get_stock_persediaan_jasa_rows($this, $tgl_jual, $uuid_unit_ajax);
			...
			if (preg_match_all('/<tr\b/i', $render['tbody'], $m)) {
				...
				if (strpos($render['tbody'], 'Tidak ada jasa persediaan') !== false) {
					$jumlah_tampil = 0;
				}
			}
			...
			if (is_array($rows) && count($rows) > 0) {
				...
				if ($bulan_baru !== '' && $bulan_lama !== '' && $bulan_baru !== $bulan_lama) {
					echo json_encode(array(
						'ok' => false,
						'message' => 'Tgl Jual tidak boleh diubah ke bulan lain selama masih ada data jasa penjualan.',
					));
					return;
				}
			}
	...
	public function create_action_simpan_jasa($uuid_penjualan = null, $id_persediaan_barang = null)
	{
		...
		if (trim((string) $data_barang->uuid_barang) === '') {
			$this->session->set_flashdata('message', 'Jasa persediaan tidak memiliki uuid_barang.');
			redirect(site_url('tbl_penjualan_jasa/create'));
			return;
		}
		...
		if ($uuid_barang_post !== '' && $uuid_barang_post !== $data_barang->uuid_barang) {
			$this->session->set_flashdata('message', 'Data jasa tidak sesuai (uuid_barang).');
			redirect(site_url('tbl_penjualan_jasa/create'));
			return;
		}

		if ($this->db->field_exists('kategori', 'persediaan') && !penjualan_is_kategori_jasa(isset($data_barang->kategori) ? $data_barang->kategori : '')) {
			$this->session->set_flashdata('message', 'Hanya item kategori Jasa yang dapat dijual melalui Input Penjualan Jasa.');
			redirect(site_url('tbl_penjualan_jasa/create'));
			return;
		}
		...
		if (!empty($data_barang->tanggal_beli)) {
			...
			if ($tgl_beli < $filter_bulan['awal'] || $tgl_beli > $filter_bulan['akhir']) {
				$this->session->set_flashdata(
					'message',
					'Jasa tidak termasuk persediaan bulan ' . $filter_bulan['bulan_label'] . ' (sesuai Tgl Jual).'
				);
				redirect(site_url('tbl_penjualan_jasa/create'));
				return;
			}
		...
		if ($uuid_penjualan == "new") {
			...
			// print_r('create_action_simpan_jasa NEW');
			// print_r("<br/>");
			// print_r("<br/>");
			// print_r($data);
			// die;
		...
		if (empty($hasil_persediaan['ok'])) {
			...
			if ($uuid_penjualan === 'new') {
				redirect(site_url('tbl_penjualan_jasa/create_action_inisiasi/new'));
			} else {
		...
		// redirect(site_url('tbl_penjualan_jasa/kasir_penjualan/' . $uuid_penjualan . '/' . $tgl_jual_X . '/' . $this->input->post('nmrkirim', TRUE)));
		redirect(site_url('tbl_penjualan_jasa/kasir_penjualan/' . $uuid_penjualan));
	}
	...
	public function kasir_penjualan($uuid_penjualan)
	{
		...
		if (!$data_penjualan_per_uuid_penjualan_first_row) {
			$this->session->set_flashdata('message', 'Detail data penjualan tidak ditemukan.');
			redirect(site_url('tbl_penjualan_jasa'));
			return;
		}
	...
	public function action_ubah_detail_nomor_kirim($NomorKirim = null, $uuid_penjualan = null)
	{
		...
		$sql_update_penjualan_by_uuid_penjualan = "UPDATE `tbl_penjualan` SET `tgl_jual`='$date_jual' , `nmrkirim`='$NomorKirim_baru', `nmrpesan`='$NomorPesan_baru', `uuid_konsumen`='$GET_uuid_konsumen', `konsumen_nama`='$nama_konsumen', `uuid_unit`='$Get_uuid_unit', `unit`='$Get_nama_unit'  WHERE `uuid_penjualan`='$uuid_penjualan' AND `barang_jasa`='jasa'";
	...
	public function create_action_nmrkirim_update_per_id_penjualan($id)
	{
		...
		$sql_data_penjualan = "SELECT * FROM `tbl_penjualan` WHERE `id`='$id' AND `barang_jasa`='jasa'";
		...
		if (empty($hasil_persediaan['ok'])) {
			...
			redirect(site_url('tbl_penjualan_jasa/kasir_penjualan/' . $Get_uuid_penjualan));
			return;
		}
	...
	private function _prepare_cetak_penjualan_jasa_data($uuid_penjualan)
	{
	...
	public function cetak_penjualan_per_uuid_penjualan($uuid_penjualan = null)
	{
		...
		$base = $this->_prepare_cetak_penjualan_jasa_data($uuid_penjualan);
		$row_cetak = $this->Tbl_penjualan_cetak_pembayaran_model->get_latest_by_uuid_penjualan($uuid_penjualan);
		...
		$data = array_merge($base, array(
			...
			'action' => site_url('tbl_penjualan_jasa/cetak_penjualan_per_uuid_penjualan_action/' . $uuid_penjualan),
	...
	public function cetak_penjualan_per_uuid_penjualan_action($uuid_penjualan = null)
	{
		if (empty($uuid_penjualan)) {
			show_404();
		}
		...
		$uuid_cetak_pembayaran = $this->Tbl_penjualan_cetak_pembayaran_model->update_by_uuid_penjualan($uuid_penjualan, $save_data);
		...
		if ($open_print === '1') {
			...
			$this->session->set_flashdata('message', 'Data cetak pembayaran berhasil disimpan dan siap dicetak.');
		} else {
			$this->session->set_flashdata('cetak_save_action', 'simpan');
			$this->session->set_flashdata('message', 'Data cetak pembayaran berhasil disimpan.');
		}
	...
	public function cetak_penjualan_per_uuid_penjualan_pdf($uuid_penjualan = null, $uuid_cetak_pembayaran = null)
	{
		...
		$base = $this->_prepare_cetak_penjualan_jasa_data($uuid_penjualan);
		$data_master_penjualan_per_uuidpenjualan = $base['data_master'];
	...
	public function update($id)
	{
		...
		if ($row) {
			$data = array(
				'button' => 'Update',
				'action' => site_url('tbl_penjualan_jasa/update_action'),
			...
			// $this->load->view('anekadharma/tbl_penjualan_jasa/tbl_penjualan_form', $data);
			$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_penjualan_jasa/adminlte310_tbl_penjualan_form_update', $data);
		} else {
			$this->session->set_flashdata('message', 'Record Not Found');
			redirect(site_url('tbl_penjualan_jasa'));
		}
	...
	public function update_penjualan($uuid_penjualan_proses)
	{
		...
		if ($row) {
			$data = array(
				...
				'action' => site_url('tbl_penjualan_jasa/update_action_proses/' . $uuid_penjualan_proses),
				'id' => set_value('id', $row->id),
				'tgl_input' => set_value('tgl_input', $row->tgl_input),
			...
			// $this->load->view('anekadharma/tbl_penjualan_jasa/tbl_penjualan_form', $data);
			$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_penjualan_jasa/adminlte310_tbl_penjualan_form_update_new', $data);
		} else {
	...
	public function excel()
	{
		...
		if ($from_datatable) {
			...
			if (empty($Tbl_penjualan_rows)) {
				$session_ids = $this->session->userdata('filter_tbl_penjualan_jasa_ids');
	...
	public function rekap($uuid_konsumen = null)
	{
		...
		if (isset($uuid_konsumen)) {
			if ($uuid_konsumen == "semua") {
				$tbl_penjualan_jasa = $this->Tbl_penjualan_model->get_all();
				$data_selection = "semua";
				$nama_konsumen_selection = "semua";
			} else {
				$tbl_penjualan_jasa = $this->Tbl_penjualan_model->get_all_by_uuid_konsumen($uuid_konsumen);
			...
			if ($this->input->post('uuid_konsumen', TRUE) == null) {
				...
				redirect(site_url('tbl_penjualan_jasa/rekap/' . $this->input->post('uuid_konsumen', TRUE)));
			}
		}
		...
		$data = array(
			'Tbl_penjualan_data' => $tbl_penjualan_jasa,
			// 'q' => $q,
			// 'pagination' => $this->pagination->create_links(),
			// 'total_rows' => $config['total_rows'],
			'start' => $start,
	...
	public function setting_kode_akun_penjualan2()
	{
		...
		$tbl_penjualan_jasa = $this->Tbl_penjualan_model->get_all_group_by_tgl_jual_nmrpesan_nmr_kirim();
		...
		// $this->load->view('anekadharma/tbl_penjualan_jasa/tbl_penjualan_list', $data);		
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_penjualan_jasa/adminlte310_tbl_penjualan_list_jurnal', $data);
	}
	...
	public function jurnal_penjualan2()
	{
		...
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_penjualan_jasa/adminlte310_tbl_penjualan_list__jurnal_penjualan', $data);
	}
	...
	public function input_kode_akun($nmrkirim = null, $Tgl_JUAL = null)
	{
		...
		$data = array(
			...
			'nmrkirim' => $nmrkirim,
			'action' => site_url('tbl_penjualan_jasa/update_kode_akun/' . $nmrkirim . '/' . $Get_date),
			'button' => 'Simpan Kode AKun',
			// 'start' => $start,
		);
		// print_r($data);
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_penjualan_jasa/adminlte310_tbl_penjualan_list_per_nmrkirim', $data);
	}
	...
	public function update_kode_akun($nmrkirim = null, $Tgl_JUAL = null)
	{
		...
		$sql = "SELECT * FROM `tbl_penjualan` WHERE `nmrkirim`='$nmrkirim' AND `barang_jasa`='jasa' ORDER BY `id`";
		...
		redirect(site_url('tbl_penjualan_jasa/setting_kode_akun_penjualan2'));
	}
	...
	public function ubah_kode_akun($nmrkirim = null)
	{
		...
		$sql = "SELECT `nmrkirim`,`kode_akun`,`kode_pl`,`kode_bb` FROM `tbl_penjualan` WHERE `nmrkirim`='$nmrkirim' AND `barang_jasa`='jasa' GROUP by `nmrkirim`,`kode_akun`";
		...
		$data = array(
			'Tbl_penjualan_data' => $data_per_nmrkirim,
			'nmrkirim' => $nmrkirim,
			'action' => site_url('tbl_penjualan_jasa/update_kode_akun/' . $nmrkirim),
