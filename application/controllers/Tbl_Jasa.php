...
class Tbl_Jasa extends CI_Controller
{
	function __construct()
	{
		parent::__construct();
		is_login();
		$this->load->model(array('Tbl_jasa_model', 'Tbl_penjualan_model', 'User_model', 'Sys_bank_model', 'Sys_status_transaksi_model', 'Tbl_penjualan_pembayaran_model',  'Sys_nama_barang_model', 'Sys_kode_akun_model', 'Sys_unit_model', 'Persediaan_model', 'Tbl_kas_kecil_model', 'Buku_besar_model', 'Tbl_penjualan_accounting_pembayaran_model', 'Tbl_penjualan_accounting_model'));
	...
	public function update_uuidpersediaan_pembelian()
	{
		...
		foreach ($this->Tbl_jasa_model->get_all() as $list_data) {

			// Cek uuid_persediaan berdasarkan uuid_spop dan uuid_barang , kemudian update field uuid_persediaan tbl_jasa dari tabel persediaan
			...
			// Update field uuid_persediaan dan id_persediaan di tabel tbl_jasa
			...
			$this->Tbl_jasa_model->update($list_data->id, $data);
	...
	{
		$this->load->view('anekadharma/tbl_jasa/tbl_jasa_list');
	}
	...
	public function index()
	{
		...
		$sql = "SELECT * FROM `tbl_jasa` WHERE `tgl_po` between '$Get_date_awal' and '$Get_date_akhir' ORDER BY `tgl_po`,`spop`,`id`";

		// print_r($this->db->query($sql)->result());
		// die;

		// $tbl_jasa = $this->Tbl_jasa_model->get_all();
		$tbl_jasa = $this->db->query($sql)->result();
		...
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_jasa/adminlte310_tbl_jasa_list', $data);
	}
	...
	public function stock($uuid_gudang = null)
	{
		...
		// tbl_jasa.uraian as barang_beli, tbl_jasa.jumlah as jumlah_belanja, tbl_jasa.harga_satuan as harga_satuan_beli, tbl_jasa.tgl_po as tgl_po,tbl_jasa.uuid_gudang as uuid_gudang, tbl_jasa.nama_gudang as nama_gudang, tbl_jasa.satuan as satuan,
		// tbl_penjualan.nama_barang as barang_jual, tbl_penjualan.jumlah as jumlah_terjual
		//                     FROM persediaan  
		//                    	left join tbl_jasa ON persediaan.uuid_barang = tbl_jasa.uuid_barang 
		//                     left join tbl_penjualan ON persediaan.uuid_barang = tbl_penjualan.uuid_barang  
		// 					WHERE tbl_jasa.uuid_gudang='$uuid_gudang' AND (persediaan.uuid_barang, persediaan.tanggal) IN (SELECT persediaan.uuid_barang, Max(persediaan.tanggal) FROM persediaan GROUP BY persediaan.uuid_barang)  
		...
		if (null !== ($this->input->post('uuid_gudang', TRUE))) {
			if ($this->input->post('uuid_gudang', TRUE) == "semua") {
				...
				// $Data_stock = $this->Tbl_jasa_model->stock();
				...
				$sql_stock = "SELECT persediaan.kode_barang as kode_barang, persediaan.namabarang as nama_barang_beli,persediaan.total_10 as jumlah_sediaan,  persediaan.hpp as harga_satuan_persediaan," . $kategori_select_sql . "
								tbl_jasa.uuid_pembelian as uuid_pembelian,tbl_jasa.uraian as barang_beli, tbl_jasa.jumlah as jumlah_belanja, tbl_jasa.harga_satuan as harga_satuan_beli,  tbl_jasa.tgl_po as tgl_po,tbl_jasa.uuid_gudang as uuid_gudang, tbl_jasa.nama_gudang as nama_gudang,  tbl_jasa.satuan as satuan,
							tbl_penjualan.nama_barang as barang_jual, tbl_penjualan.jumlah as jumlah_terjual
                            FROM persediaan  
                           	" . $kategori_join_sql . "left join tbl_jasa ON persediaan.uuid_barang = tbl_jasa.uuid_barang 
                            left join tbl_penjualan ON persediaan.uuid_barang = tbl_penjualan.uuid_barang  
				...
				// $Data_stock = $this->Tbl_jasa_model->stock_by_gudang($uuid_gudang);
				$sql_stock = "SELECT persediaan.kode_barang as kode_barang, persediaan.namabarang as nama_barang_beli,persediaan.total_10 as jumlah_sediaan, persediaan.hpp as harga_satuan_persediaan," . $kategori_select_sql . "
								tbl_jasa.uuid_pembelian as uuid_pembelian,tbl_jasa.uraian as barang_beli, tbl_jasa.jumlah as jumlah_belanja, tbl_jasa.harga_satuan as harga_satuan_beli, tbl_jasa.tgl_po as tgl_po,tbl_jasa.uuid_gudang as uuid_gudang, tbl_jasa.nama_gudang as nama_gudang,tbl_jasa.satuan as satuan,
							tbl_penjualan.nama_barang as barang_jual, tbl_penjualan.jumlah as jumlah_terjual
							FROM persediaan  
		...
		if (isset($uuid_gudang)) {
			...
			" . $kategori_select_sql . "
					-- 	tbl_jasa.uuid_pembelian as uuid_pembelian,
					-- 	tbl_jasa.uraian as barang_beli, 
					-- 	tbl_jasa.jumlah as jumlah_belanja, 
					-- 	tbl_jasa.harga_satuan as harga_satuan_beli, 
					-- 	tbl_jasa.tgl_po as tgl_po, 
					-- 	tbl_jasa.uuid_gudang as uuid_gudang, 
					-- 	tbl_jasa.nama_gudang as nama_gudang,
					-- 	tbl_jasa.satuan as satuan,
		...
		$data = array(
			'action_cari_gudang' => site_url('tbl_jasa/stock'),
	...
	public function create_pembayaran($uuid_spop = null, $from_pembelian_page = null)
	{
		...
		$row_per_uuid_spop = $this->Tbl_jasa_model->get_by_uuid_spop($uuid_spop);
		$RESULT_per_uuid_spop = $this->Tbl_jasa_model->get_by_uuid_spop_ALL_result($uuid_spop);

		$data = array(
			'data_ALL_per_SPOP' => $RESULT_per_uuid_spop,
			'button' => 'Simpan',
			'action' => site_url('tbl_jasa/create_pembayaran_action/' . $uuid_spop),
			'id' => set_value('id'),
		...
		// $this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_jasa/adminlte310_tbl_jasa_form_spop_pengajuan_bayar', $data);
		...
		$RESULT_get_total_nominal_per_spop = $this->Tbl_jasa_model->get_total_nominal_per_spop($uuid_spop);
		...
		$Query_data_pengajuan_bayar_by_uuid_spop = $this->db->get('tbl_jasa_pengajuan_bayar');
		...
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_jasa/adminlte310_tbl_jasa_cetak_pengajuan_pembayaran', $data);
	}
	...
	public function create_pembayaran_action($uuid_spop = null)
	{
		...
		$uuid_pengajuan_bayar_terproses = $this->tbl_jasa_pengajuan_bayar_model->insert($data);
		...
		$this->db->where('uuid_spop', $uuid_spop);
		$Get_data_pembayaran_pembelian = $this->db->get('tbl_jasa_pengajuan_bayar');
		...
		if ($Get_data_pembayaran_pembelian->num_rows() > 0) {
			...
			$sql_pembayaran = "SELECT `uuid_spop`, sum(`nominal_pengajuan`) as total_sudah_terbayar FROM `tbl_jasa_pengajuan_bayar` WHERE `uuid_spop`='$uuid_spop' GROUP by `uuid_spop`";
			...
			$sql_data_pembelian = "SELECT `uuid_spop`,`spop`,`jumlah`,`harga_satuan` FROM `tbl_jasa` WHERE `uuid_spop`='$uuid_spop'";
			...
			if ($Data_Pembayaran_uuid_spop->total_sudah_terbayar >= $jumlah_tagihan_total) {
				...
				$this->Tbl_jasa_model->update_statuslu_per_spop($uuid_spop, $data);
			}
		}
		...
		redirect(site_url('tbl_jasa/success_pengajuan/' . $uuid_pengajuan_bayar_terproses));
	}

	public function success_pengajuan($uuid_pengajuan_bayar_terproses)
	{

		$row_per_uuid_pengajuan_bayar_terproses = $this->tbl_jasa_pengajuan_bayar_model->get_by_uuid_pengajuan_bayar($uuid_pengajuan_bayar_terproses);
		...
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_jasa/adminlte310_tbl_jasa_cetak_pengajuan_pembayaran_view', $data);
	}
	...
	public function cetak_pengajuan_bayar_per_spop($uuid_pengajuan_bayar_terproses = null)
	{
		...
		$html = $this->load->view('anekadharma/tbl_jasa/adminlte310_tbl_jasa_cetak_pengajuan_pembayaran_print.php', $data, true);
	...
	public function pembayaran()
	{
		...
		// $Data_supplier_tagihan = $this->Tbl_jasa_model->supplier_tagihan();
		// $Data_supplier_tagihan = $this->tbl_jasa_pengajuan_bayar_model->get_all();


		$sql = "SELECT tbl_jasa_a.uuid_spop as uuid_spop, 
        tbl_jasa_a.spop as spop,
        tbl_jasa_a.jumlah as jumlah,
        tbl_jasa_a.harga_satuan as harga_satuan,
        sum(tbl_jasa_a.harga_total) as total_pembelian,
        tbl_jasa_a.supplier_nama as supplier_nama,
        tbl_jasa_pengajuan_bayar_a.uuid_pengajuan_bayar as uuid_pengajuan_bayar,
        tbl_jasa_pengajuan_bayar_a.nominal_pengajuan as nominal_pengajuan


        FROM tbl_jasa tbl_jasa_a 
		
		left join   tbl_jasa_pengajuan_bayar  tbl_jasa_pengajuan_bayar_a ON  tbl_jasa_pengajuan_bayar_a.uuid_spop = tbl_jasa_a.uuid_spop

		group by tbl_jasa_a.uuid_spop,tbl_jasa_pengajuan_bayar_a.uuid_pengajuan_bayar
        ";
	...
	public function pembayaran_ke_supplier()
	{
		...
		$sql = "SELECT 
		tbl_jasa_a.id as id, 
		tbl_jasa_a.uuid_spop as uuid_spop, 
        tbl_jasa_a.tgl_po as tgl_po,
        tbl_jasa_a.spop as spop,
        tbl_jasa_a.jumlah as jumlah,
        tbl_jasa_a.harga_satuan as harga_satuan,
        sum(tbl_jasa_a.harga_total) as total_pembelian,
        tbl_jasa_a.supplier_nama as supplier_nama,
        tbl_jasa_a.statuslu as statuslu,
        -- tbl_jasa_pengajuan_bayar_a.uuid_pengajuan_bayar as uuid_pengajuan_bayar,
        -- tbl_jasa_pengajuan_bayar_a.nominal_pengajuan as nominal_pengajuan
		tbl_jasa_a.kas_bank as kas_bank
        FROM tbl_jasa tbl_jasa_a 
		-- left join   tbl_jasa_pengajuan_bayar  tbl_jasa_pengajuan_bayar_a ON  tbl_jasa_pengajuan_bayar_a.uuid_spop = tbl_jasa_a.uuid_spop
		group by tbl_jasa_a.uuid_spop
		order by tbl_jasa_a.tgl_po asc
        ";
		// return $this->db->query($sql)->result();
		// print_r($this->db->query($sql)->result());
	...
	public function update_pembayaran_ke_supplier_action($id_data = null)
	{
		...
		// 	$Query_data_pengajuan_bayar_by_id = $this->db->get('tbl_jasa_pengajuan_bayar')->row();
		...
		$uuid_pengajuan_bayar_terproses = $this->tbl_jasa_pengajuan_bayar_model->update($id_data, $data);
		...
		if ($Get_data_pembayaran_pembelian->num_rows() > 0) {
			...
			$sql_pembayaran = "SELECT `uuid_spop`, sum(`nominal_pengajuan`) as total_sudah_terbayar FROM `tbl_jasa_pengajuan_bayar` WHERE `id`='$id_data' GROUP by `uuid_spop`";
	...
	public function update_pembayaran_ke_supplier($id_data = null)
	{
		...
		$row_per_uuid_spop = $this->Tbl_jasa_model->get_by_uuid_spop($Query_data_pengajuan_bayar_by_id->uuid_spop);
		...
		$data = array(
			'data_ALL_per_SPOP' => $RESULT_per_uuid_spop,
			'button' => 'Update',
			'action' => site_url('tbl_jasa/update_pembayaran_ke_supplier_action/' . $id_data),
	...
	{
		$row = $this->tbl_jasa_pengajuan_bayar_model->get_by_id($id_data);
		...
		if ($row) {
			$this->tbl_jasa_pengajuan_bayar_model->delete($id_data);
			$this->session->set_flashdata('message', 'Delete Record Success');
			redirect(site_url('tbl_jasa/pembayaran_ke_supplier'));
		} else {
	...
	public function tagihan_per_uuid_konsumen($uuid_konsumen = null)
	{
		...
		$data = array(
			'button' => 'Simpan',
			'action_nominal' => site_url('tbl_jasa/create_pembayaran_per_uuid_konsumen_by_nominal_action/' . $uuid_konsumen),
			'action_pertransaksi' => site_url('tbl_jasa/create_pembayaran_per_uuid_konsumen_by_transaksi_action/' . $uuid_konsumen),
	...
	public function create_pembayaran_per_uuid_konsumen_by_nominal_action($uuid_konsumen = null)
	{
		...
		redirect(site_url('tbl_jasa/pembayaran_dari_konsumen'));
	}
	...
	public function create_pembayaran_per_uuid_konsumen_by_transaksi_action($uuid_konsumen = null)
	{
		...
		redirect(site_url('tbl_jasa/tagihan_per_uuid_konsumen/' . $uuid_konsumen));
	}
	...
	public function pilih_proses_bayar_pertransaksi($uuid_konsumen_selected = null, $uuid_penjualan_proses_selected = null)
	{
		...
		$this->db->query($sql);
		redirect(site_url('tbl_jasa/tagihan_per_uuid_konsumen/' . $uuid_konsumen_selected));
	}
	...
	public function read($id)
	{
		...
		if ($row) {
			...
			$this->load->view('anekadharma/tbl_jasa/tbl_jasa_read', $data);
		} else {
			$this->session->set_flashdata('message', 'Record Not Found');
	...
	public function create()
	{
		$data = array(
			'button' => 'Simpan',
			'action' => site_url('tbl_jasa/create_action_uuid_spop'),
			'id' => set_value('id'),
		...
		// $this->load->view('anekadharma/tbl_jasa/tbl_jasa_form', $data);
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_jasa/adminlte310_tbl_jasa_form', $data);
	}
	...
	public function create_action()
	{
		...
		if (($this->form_validation->run() == FALSE) or is_null($this->input->post('uuid_supplier', TRUE))  or is_null($this->input->post('uuid_konsumen', TRUE))) {
			...
			// GET SPOP
			$sql = "SELECT MAX(`spop`) as maxSPOP FROM `tbl_jasa`";
	...
	public function create_add_uraian_update($uuid_spop = null)
	{
		...
		$data = array(
			...
			'action' => site_url('tbl_jasa/create_action_detail_uuid_spop_update/' . $uuid_spop),
			'action_ubah_detail_spop' => site_url('tbl_jasa/create_action_detail_uuid_spop_update/' . $uuid_spop),
			'id' => set_value('id'),
			'tgl_po' => $row_per_uuid_spop->tgl_po,
			// 'nmrsj' => $row_per_uuid_spop->nmrsj,
			'nmrfakturkwitansi' => $row_per_uuid_spop->nmrfakturkwitansi,
			...
			// 'tgl_bayar' => $row_per_uuid_spop->tgl_bayar,
			// 'id_usr' => $row_per_uuid_spop->,
			'action_ubah_per_id' => site_url('tbl_jasa/create_action_uuid_spop_update_per_id/'),
			'action_tambah_barang_per_spop' => site_url('tbl_jasa/create_action_tambah_barang_per_spop/'),
		);
		...
		// $this->load->view('anekadharma/tbl_jasa/tbl_jasa_form', $data);
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_jasa/adminlte310_tbl_jasa_form_spop_ready', $data);
	}
	...
	public function create_action_detail_uuid_spop_update($uuid_spop = null)
	{
		...
		if ($row_per_uuid_spop->spop <> $this->input->post('spop', TRUE)) {
			...
			// redirect(site_url('tbl_jasa/create_add_uraian_update/' . $uuid_spop));
			...
			$Get_record_spop = $this->db->get('tbl_jasa');
			...
			// die;
			redirect(site_url('tbl_jasa/create_add_uraian_update/' . $uuid_spop_processing));
		} else {
			...
			$this->Tbl_jasa_model->update_proses_per_spop($uuid_spop, $data_update_spop);
		...
		// $this->db->where('uuid_spop', $uuid_spop);
		// $get_data_spop = $this->db->get('tbl_jasa');
	...
	public function create_action_tambah_barang_per_spop($uuid_spop = null)
	{
		...
		$this->Tbl_jasa_model->insert($data); // insert untuk data lanjutan uuid_spop sudah ada
	...
	public function create_action_uuid_spop_update_per_id($id)
	{
		...
		$sql_data_pembelian = "SELECT * FROM `tbl_jasa` WHERE `id`='$id'";
		...
		$this->Tbl_jasa_model->update($id, $data);
		...
		redirect(site_url('tbl_jasa/create_add_uraian_update/' . $get_uuid_spop));
	}
	...
	public function create_action_uuid_spop_update($uuid_spop = null)
	{
		...
		if ($uuid_spop) {
			...
			$get_uuid_spop_generating = $this->Tbl_jasa_model->insert_spop($data);
			// print_r("<br/>");
		...
		redirect(site_url('tbl_jasa/create_add_uraian/' . $get_uuid_spop_generating));
	}
	...
	public function create_add_uraian($uuid_spop = null)
	{
		...
		$data = array(
			'data_ALL_per_SPOP' => $RESULT_per_uuid_spop,
			'button' => 'Simpan',
			'action' => site_url('tbl_jasa/create_action_uuid_spop/' . $uuid_spop),
			'id' => set_value('id'),
			'tgl_po' => $row_per_uuid_spop->tgl_po,
			// 'nmrsj' => $row_per_uuid_spop->nmrsj,
			'nmrfakturkwitansi' => $row_per_uuid_spop->nmrfakturkwitansi,
	...
	public function create_action_uuid_spop($uuid_spop = null)
	{
		...
		if ($uuid_spop) {
			...
			$GET_ID_PEMBELIAN_AFTER_INSERT = $this->Tbl_jasa_model->insert($data); // insert untuk data lanjutan uuid_spop sudah ada
			$get_uuid_spop_generating = $uuid_spop;
			...
			$GET_ID_PEMBELIAN_AFTER_INSERT = $this->Tbl_jasa_model->insert_spop($data);
			// $GET_ID_PEMBELIAN_AFTER_INSERT = $this->Tbl_jasa_model->insert($data);
			...
			$GET_DATA_PEMBELIAN = $this->db->get('tbl_jasa');
		...
		// UPDATE tbl_jasa : ID_PERSEDIAAN DAN UUID_PERSEDIAAN DARI TABEL PERSEDIAAN DENGAN FILTER UUID_SPOP DARI TABEL PERSEDIAAN
		...
		$sql_update_tbl_jasa = "UPDATE `tbl_jasa` SET `uuid_persediaan`='$GET_uuid_persediaan',`id_persediaan_barang`='$GET_ID_PERSEDIAAN' WHERE `id`='$GET_ID_PEMBELIAN_AFTER_INSERT'";

		$this->db->query($sql_update_tbl_jasa);
		...
		// $this->db->where('id', $GET_ID_PEMBELIAN_AFTER_INSERT);
		// $this->db->update('tbl_jasa'); // table name

		// END OF UPDATE tbl_jasa : ID_PERSEDIAAN DAN UUID_PERSEDIAAN DARI TABEL PERSEDIAAN DENGAN FILTER UUID_SPOP DARI TABEL PERSEDIAAN
	...
	public function simpan_data_spop($uuid_spop = null)
	{
		...
		$this->Tbl_jasa_model->update_proses_per_spop($uuid_spop, $data);
	...
	public function cetak_belanja_per_spop($uuid_spop = null)
	{
		...
		// 2.b. PERSIAPAN DATA
		$data = array(
			'data_belanja' => $this->Tbl_jasa_model->get_by_spop($uuid_spop, $data),
		);

		// 2.C. MENAMPILKAN FILE DATA
		// $data = array_merge($data);
		$html = $this->load->view('anekadharma/tbl_jasa/adminlte310_cetak_pembelian.php', $data, true);
	...
	public function update_per_spop($uuid_spop)
	{
		...
		$data_per_uuidspop = $this->Tbl_jasa_model->get_by_uuid_spop($uuid_spop);
		// print_r($data_per_uuidspop->spop);
		// die;

		$tbl_jasa = $this->Tbl_jasa_model->get_by_spop($data_per_uuidspop->spop);
		$start = 0;
		$data = array(
		...
		// $this->template->load('anekadharma/adminlte310_anekadharma', 'anekadharma/tbl_jasa/adminlte310_tbl_jasa_list_per_spop', $data);
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_jasa/adminlte310_tbl_jasa_list_per_spop', $data);
	}
	...
	public function update_per_id($uuid_pembelian)
	{
		...
		// $data_per_uuidspop = $this->Tbl_jasa_model->get_by_id($uuid_spop);
		// print_r($data_per_uuidspop->spop);
		// die;
		...
		$get_id_pembelian = $this->Tbl_jasa_model->get_by_uuid_pembelian($uuid_pembelian);
		// print_r($get_id_pembelian->id);
		// print_r("<br/>");

		$tbl_jasa = $this->Tbl_jasa_model->get_by_id($get_id_pembelian->id);
		// print_r($tbl_jasa);
		// print_r("<br/>");
		...
		$data = array(
			...
			'tgl_po' => date($tbl_jasa->tgl_po),
			'uuid_supplier' => $tbl_jasa->uuid_supplier,
			'supplier_nama' => $tbl_jasa->supplier_nama,
			'statuslu' => $tbl_jasa->statuslu,
			'kas_bank' => $tbl_jasa->kas_bank,
			'spop' => $tbl_jasa->spop,
			'nmrfakturkwitansi' => $tbl_jasa->nmrfakturkwitansi,
			'uuid_gudang' => $tbl_jasa->uuid_gudang,
			'nama_gudang' => $tbl_jasa->nama_gudang,

			'uraian' => $tbl_jasa->uraian,
			'uuid_barang' => $tbl_jasa->uuid_barang,
			'kode_barang' => $tbl_jasa->kode_barang,

			'jumlah' => $tbl_jasa->jumlah,
			'satuan' => $tbl_jasa->satuan,
			'harga_satuan' => $tbl_jasa->harga_satuan,
		...
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_jasa/adminlte310_tbl_jasa_form_per_id', $data);
	}
	...
	public function update_action()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			// print_r($data);
			// die;
			$this->Tbl_jasa_model->update($this->input->post('id', TRUE), $data);
	...
	public function update_status_per_spop($uuid_spop = null)
	{
		...
		// $tbl_jasa = $this->Tbl_jasa_model->get_by_spop($spop);
		$tbl_jasa = $this->Tbl_jasa_model->get_by_spop($data_per_uuidspop->spop);

		// SELECT `status_spop` FROM `tbl_jasa` WHERE `uuid_spop`="53d056417ed111ef95300021ccc9061e";
		...
		$data = array(
			...
			'action' => site_url('tbl_jasa/update_status_per_spop_action/' . $uuid_spop),
			'button' => 'Simpan',
			'start' => $start,
		);
		// print_r($data);
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_jasa/adminlte310_tbl_jasa_list_per_spop_status', $data);
	}
	...
	public function update_status_per_spop_action($uuid_spop)
	{
		...
		$this->Tbl_jasa_model->update_status_per_spop($uuid_spop, $data);
		$this->session->set_flashdata('message', 'Update Record Success');
		redirect(site_url('tbl_jasa'));
		// }
	}
	...
	public function delete_per_spop($uuid_spop)
	{
		...
		// print_r("<br/>");
		// $row = $this->Tbl_jasa_model->get_by_id($uuid_spop);
		$data_per_uuidspop = $this->Tbl_jasa_model->get_by_uuid_spop($uuid_spop);
		...
		if ($data_per_uuidspop) {
			// print_r("Ada data");
			$this->Tbl_jasa_model->delete_by_uuid_spop($uuid_spop);
			// die;
			$this->session->set_flashdata('message', 'Delete Record Success');
			redirect(site_url('tbl_jasa'));
		} else {
	...
	public function delete_by_uuid_pembelian_from_per_spop_update($uuid_pembelian = null, $get_uuid_spop = null)
	{
		...
		$get_id_pembelian = $this->Tbl_jasa_model->get_by_uuid_pembelian($uuid_pembelian);
		$get_id_persediaan_di_tbl_jasa = $get_id_pembelian->id_persediaan_barang;


		$row = $this->Tbl_jasa_model->get_by_id($get_id_pembelian->id);

		if ($row) {

			// Hapus record di tabel pembelian
			$this->Tbl_jasa_model->delete($get_id_pembelian->id);

			// hapus record di tabel persediaan
			$this->Persediaan_model->delete($get_id_persediaan_di_tbl_jasa);
	...
	public function delete_by_uuid_spop($uuid_spop)
	{
		...
		// $get_data_uuid_spop = $this->Tbl_jasa_model->get_by_uuid_spop($uuid_spop);

		$row = $this->Tbl_jasa_model->get_by_uuid_spop($uuid_spop);
	...
	public function pecah_satuan_proses($uuid_persediaan = null)
	{
		...
		$sql_pembelian_barang = "SELECT `uuid_persediaan`,`id_persediaan_barang`,`uuid_barang`,`uraian`,sum(`jumlah`) as jumlah_beli FROM `tbl_jasa` WHERE `id_persediaan_barang`='$Get_id_persediaan_barang'";
		...
		$data = array(
			...
			'action' => site_url('tbl_jasa/pecah_satuan_action/' . $uuid_persediaan),
			'button' => 'Simpan',
		...
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_jasa/adminlte310_tbl_jasa_form_pecah_satuan_barang', $data);
	}
	...
	public function rollback_satuan_proses($uuid_persediaan = null)
	{
		...
		// $Data_Barang = $this->Tbl_jasa_model->get_by_uuid_pembelian($uuid_pembelian);
		// $Data_Barang = $this->Persediaan_model->get_by_uuid_persediaan($uuid_persediaan);

		// print_r($Data_Barang);

		// get data pembelian awal ==> jika tidak ada di data tbl_jasa maka termasuk data stock persediaan
		// get_uuid_barang dengan filter uuid_barang di tbl_jasa
		...
		sum(tbl_jasa.jumlah) as sum_jumlah_beli,
		sum(tbl_penjualan.jumlah) as sum_jumlah_jual
				...
				Group by persediaan.uuid_barang,tbl_jasa.uuid_barang,tbl_penjualan.uuid_barang
		...
		$sql_barang_pecah_satuan = "SELECT * FROM tbl_jasa_pecah_satuan where tbl_jasa_pecah_satuan.uuid_barang_baru='$GET_uuid_barang' ";
		...
		$data = array(
			'Data_Barang' => $Data_Barang,
			'action' => site_url('tbl_jasa/rollback_satuan_action/' . $Get_uuid_pecah_satuan_proses),
			'button' => 'Simpan',
		...
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_jasa/adminlte310_tbl_jasa_form_rollback_barang', $data);
	}
	...
	public function rollback_satuan_action($Get_uuid_pecah_satuan_proses = null)
	{
		...
		// Update tbl_jasa_pecah_satuan ==> mengurangi jumlah yang terpecah
		...
		$sql_barang_pecah_satuan = "SELECT * FROM `tbl_jasa_pecah_satuan` WHERE `uuid_pecah_satuan`='$Get_uuid_pecah_satuan_proses'";
		$Data_Barang_pecah_satuan = $this->db->query($sql_barang_pecah_satuan)->row();
		...
		// // Source Data Barang Pecah
		$sql_barang_pecah_satuan = "UPDATE `tbl_jasa_pecah_satuan` SET `jumlah` = $Get_jumlah_akhir_proses , `jumlah_barang_baru` = $Get_jumlah_update_jumlah_terpecah WHERE `uuid_pecah_satuan`='$Get_uuid_pecah_satuan_proses'";
		...
		// print_r("Selesai");
		// die;
		redirect(site_url('tbl_jasa/pecah_satuan'));
	}
	...
	public function pecah_satuan($uuid_gudang = null)
	{
		...
		if (isset($uuid_gudang)) {
			...
			$sql_stock = "SELECT persediaan.uuid_persediaan as uuid_persediaan, persediaan.uuid_barang as uuid_barang, persediaan.kode_barang as kode_barang, persediaan.namabarang as nama_barang_beli,persediaan.total_10 as jumlah_sediaan, persediaan.hpp as harga_satuan_persediaan, persediaan.tanggal as tanggal , persediaan.satuan as satuan , sum(tbl_jasa_a.jumlah) as sum_jumlah_beli, sum(tbl_penjualan_a.jumlah) as sum_jumlah_jual
						FROM persediaan persediaan_a   
						   left join tbl_jasa tbl_jasa_a ON persediaan.uuid_barang = tbl_jasa_a.uuid_barang 
						left join tbl_penjualan tbl_penjualan_a ON persediaan.uuid_barang = tbl_penjualan_a.uuid_barang  
						WHERE tbl_jasa_a.uuid_gudang='$uuid_gudang' AND (persediaan.uuid_barang, persediaan.tanggal) IN (SELECT persediaan.uuid_barang, Max(persediaan.tanggal) FROM persediaan persediaan_a GROUP BY persediaan.uuid_barang)  
						ORDER BY persediaan.namabarang ASC";
			...
			// sum(tbl_jasa.jumlah) as sum_jumlah_beli,
			// sum(tbl_penjualan.jumlah) as sum_jumlah_jual,
			// tbl_jasa_pecah_satuan.jumlah as jumlah_terpecah,
			// tbl_jasa_pecah_satuan.jumlah_barang_baru as jumlah_setelah_terpecah,
			// tbl_jasa_pecah_satuan.uuid_barang as uuid_barang_pecah,
			// tbl_jasa_pecah_satuan.uuid_barang_baru as uuid_barang_baru
			// 		FROM persediaan  
			// 		left join tbl_jasa ON persediaan.uuid_barang = tbl_jasa.uuid_barang 
			// 		left join tbl_penjualan ON persediaan.uuid_barang = tbl_penjualan.uuid_barang  
			// 		left join tbl_jasa_pecah_satuan ON persediaan.uuid_barang = tbl_jasa_pecah_satuan.uuid_barang  
			// 		-- WHERE (persediaan.uuid_barang, persediaan.tanggal) IN (SELECT persediaan.uuid_barang, Max(persediaan.tanggal) FROM persediaan persediaan_a GROUP BY persediaan.uuid_barang)  
			// 		Group by persediaan.uuid_barang,tbl_jasa.uuid_barang,tbl_penjualan.uuid_barang
			// 		ORDER BY persediaan.namabarang ASC";
		...
		$data = array(
			'action_cari_gudang' => site_url('tbl_jasa/pecah_satuan'),
			'Data_stock' => $Data_stock,
		);
	...
	public function pecah_satuan_action($uuid_persediaan)
	{
		...
		// Proses simpan ke tbl_jasa menjadi barang baru
		...
		$data_tbl_jasa_pecah_satuan = array(
		...
		// print_r("tbl_jasa_pecah_satuan: ");
		// print_r("<br/>");
		// print_r($data_tbl_jasa_pecah_satuan);
		// print_r("<br/>");
		// print_r("<br/>");
		// print_r("<br/>");
		// die;

		$this->tbl_jasa_pecah_satuan_model->insert($data_tbl_jasa_pecah_satuan);
	...
	public function jurnal_pembelian()
	{
		...
		$data = array(
			'tbl_jasa_data' => $tbl_jasa,
			'start' => $start,
			'action_by_bulan' => site_url('tbl_jasa/jurnal_pembelian_per_bulan'),
		);
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_jasa/adminlte310_jurnal_pembelian', $data);
	}
	...
	public function setting_kode_akun_pembelian2()
	{
		...
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_jasa/adminlte310_tbl_jasa_setting_kode_akun', $data);
	}
	public function jurnal_pembelian2()
	{
		...
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_jasa/adminlte310_tbl_jasa_list__jurnal_pembelian', $data);
	}
	...
	public function jurnal_pembelian2_cekBelumAdaKodeAkun()
	{
		...
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_jasa/adminlte310_tbl_jasa_list_jurnal', $data);
	}
	...
	public function input_kode_akun($uuid_spop = null)
	{
		...
		$data = array(
			'tbl_jasa_data' => $tbl_jasa,
			'spop' => $data_per_uuidspop->spop,
			'action' => site_url('tbl_jasa/update_kode_akun/' . $uuid_spop),
			'button' => 'Simpan Kode AKun',
			// 'start' => $start,
		);
		// print_r($data);
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_jasa/adminlte310_tbl_jasa_list_per_spop_kode_akun', $data);
	}
	...
	public function update_kode_akun($uuid_spop = null)
	{
		...
		$data_Pembelian_by_uuid_spop = $this->Tbl_jasa_model->get_by_uuid_spop_ALL_result($uuid_spop);
		...
		redirect(site_url('tbl_jasa/setting_kode_akun_pembelian2/'));
	}
	...
	public function ubah_kode_akun($uuid_spop = null)
	{
		...
		// print_r($tbl_jasa->kode_akun);
		// print_r("<br/>");
		// print_r("<br/>");


		$sql = "SELECT `spop`,`kode_akun`,`kode_pl`,`kode_bb` FROM `tbl_jasa` WHERE `uuid_spop`='$uuid_spop' GROUP by `uuid_spop`,`kode_akun`";
	...
	public function jurnal_pembelian_per_bulan()
	{
		...
		$sql = "SELECT * FROM tbl_jasa WHERE tgl_po >= '" . $from_date . "' AND tgl_po <= '" . $to_date . "' ORDER by id DESC";
	...
	public function kas_kecil()
	{
		...
		$sql = "SELECT * FROM tbl_jasa where kas_bank='kas' union ALL SELECT * FROM tbl_jasa_kas_kecil ";
		...
		$data = array(
			...
			'tbl_jasa_data' => $tbl_jasa,
			'start' => $start,
			'action_by_bulan' => site_url('tbl_jasa/kas_kecil_per_bulan'),
		);
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_jasa/adminlte310_kas_kecil', $data);
	}
	...
	public function kas_kecil_per_bulan()
	{
		...
		$sql = "SELECT * FROM tbl_jasa where kas_bank='kas' and tgl_po >= '" . $from_date . "' AND tgl_po <= '" . $to_date . "' UNION ALL SELECT * FROM tbl_jasa_kas_kecil where  tgl_po >= '" . $from_date . "' AND tgl_po <= '" . $to_date . "'";
	...
	public function buku_kas()
	{
		...
		$sql = "SELECT tgl_po as tgl_transaksi, uraian as nama_barang,proses_transaksi,kode_akun,jumlah,harga_satuan FROM tbl_jasa where tgl_po >= '" . $from_date . "' AND tgl_po <= '" . $to_date . "' UNION ALL SELECT tgl_jual as tgl_transaksi, nama_barang as nama_barang,proses_transaksi,kode_akun,jumlah,harga_satuan FROM tbl_penjualan  where  tgl_jual >= '" . $from_date . "' AND tgl_jual <= '" . $to_date . "'  order by tgl_transaksi";
		...
		$data = array(
			...
			'action_by_bulan' => site_url('tbl_jasa/buku_kas_per_bulan'),
		);
	...
	public function buku_kas_per_bulan()
	{
		...
		$sql = "SELECT tgl_po as tgl_transaksi, uraian as nama_barang,proses_transaksi FROM tbl_jasa where tgl_po >= '" . $from_date . "' AND tgl_po <= '" . $to_date . "' UNION ALL SELECT tgl_jual as tgl_transaksi, nama_barang as nama_barang,proses_transaksi FROM tbl_penjualan  where  tgl_jual >= '" . $from_date . "' AND tgl_jual <= '" . $to_date . "'  order by tgl_transaksi";
