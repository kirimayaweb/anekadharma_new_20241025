...
class Tbl_pembelian_jasa extends CI_Controller
{
	function __construct()
	{
		parent::__construct();
		is_login();
		$this->load->model(array('Tbl_pembelian_jasa_model', 'Tbl_penjualan_model', 'Tbl_pembelian_pengajuan_bayar_model', 'User_model', 'Sys_bank_model', 'Sys_status_transaksi_model', 'Tbl_penjualan_pembayaran_model', 'Tbl_pembelian_pecah_satuan_model', 'Sys_nama_barang_model', 'Sys_kode_akun_model', 'Sys_unit_model', 'Persediaan_model', 'Tbl_kas_kecil_model', 'Buku_besar_model', 'Tbl_penjualan_accounting_pembayaran_model', 'Tbl_penjualan_accounting_model'));
		$this->Tbl_pembelian_model = $this->Tbl_pembelian_jasa_model;
	...
	private function _build_pembelian_jasa_list_data($Get_date_awal, $Get_date_akhir)
	{
		...
		if (!empty($uuid_spop_list)) {
			foreach (array_chunk($uuid_spop_list, 500) as $uuid_chunk) {
				$chunk_grouped = $this->Tbl_pembelian_pengajuan_bayar_model->get_grouped_by_uuid_spop_in($uuid_chunk);
				foreach ($chunk_grouped as $uuid => $items) {
					$pengajuan_by_uuid_spop[$uuid] = $items;
				}
				$chunk_sums = $this->Tbl_pembelian_pengajuan_bayar_model->get_sum_nominal_grouped_by_uuid_spop_in($uuid_chunk);
				foreach ($chunk_sums as $uuid => $total) {
					$pengajuan_sum_by_uuid_spop[$uuid] = $total;
				}
			}
		}
	...
	private function _build_persediaan_from_pembelian_jasa_row($row)
	{
	...
	private function _sum_total_beli_by_link_jasa($id_persediaan, $uuid_persediaan)
	{
		...
		$this->db->from('tbl_pembelian_jasa');
		if ($id_persediaan > 0 && $uuid_persediaan !== '') {
			$this->db->group_start();
	...
	private function _sync_persediaan_for_pembelian_jasa_id($id_pembelian)
	{
		...
		$row = $this->Tbl_pembelian_jasa_model->get_by_id($id_pembelian);
		if (!$row) {
			return false;
		}

		$data_persediaan = $this->_build_persediaan_from_pembelian_jasa_row($row);
		...
		if ($id_persediaan > 0) {
			...
			if ($existing) {
				...
				$total_beli = $this->_sum_total_beli_by_link_jasa($id_persediaan, $uuid_persediaan);
				if ($total_beli > 0) {
					$data_persediaan['beli'] = $total_beli;
					$data_persediaan['sa'] = $total_beli;
					$data_persediaan['total_10'] = $total_beli;
					$data_persediaan['nilai_persediaan'] = $total_beli * $this->_parse_decimal_db(isset($row->harga_satuan) ? $row->harga_satuan : 0);
				}
				$this->Persediaan_model->update($id_persediaan, $data_persediaan);
				if ($uuid_persediaan !== '') {
					$this->Tbl_pembelian_jasa_model->update($id_pembelian, array(
		...
		if ($new_id_persediaan > 0) {
			...
			$total_beli = $this->_sum_total_beli_by_link_jasa($new_id_persediaan, $new_uuid_persediaan);
			if ($total_beli > 0) {
				$this->Persediaan_model->update($new_id_persediaan, array(
					'beli' => $total_beli,
					'sa' => $total_beli,
					'total_10' => $total_beli,
					'nilai_persediaan' => $total_beli * $this->_parse_decimal_db(isset($row->harga_satuan) ? $row->harga_satuan : 0),
				));
			}
		}
	...
	private function _resync_or_cleanup_persediaan_link_jasa($id_persediaan, $uuid_persediaan)
	{
		$id_persediaan = (int) $id_persediaan;
		$uuid_persediaan = trim((string) $uuid_persediaan);
		...
		if ($row && isset($row->id)) {
			$this->_sync_persediaan_for_pembelian_jasa_id((int) $row->id);
			return;
		}
	...
	private function _get_pembelian_jasa_rows_for_excel_by_ids(array $ids)
	{
	...
	private function _get_pembelian_jasa_rows_for_excel($Get_date_awal, $Get_date_akhir)
	{
	...
	public function create_pembayaran($uuid_spop = null, $from_pembelian_page = null)
	{
		...
		// $this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_pembelian_jasa/adminlte310_tbl_pembelian_form_spop_pengajuan_bayar', $data);
		...
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_pembelian_jasa/adminlte310_tbl_pembelian_cetak_pengajuan_pembayaran', $data);
	}
	...
	public function create_pembayaran_action($uuid_spop = null)
	{
		...
		if ($Get_data_pembayaran_pembelian->num_rows() > 0) {
			...
			$sql_data_pembelian = "SELECT `uuid_spop`,`spop`,`jumlah`,`harga_satuan` FROM `tbl_pembelian_jasa` WHERE `uuid_spop`='$uuid_spop'";
	...
	public function success_pengajuan($uuid_pengajuan_bayar_terproses)
	{
		...
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_pembelian_jasa/adminlte310_tbl_pembelian_cetak_pengajuan_pembayaran_view', $data);
	}
	...
	public function cetak_pengajuan_bayar_per_spop($uuid_pengajuan_bayar_terproses = null)
	{
		...
		$html = $this->load->view('anekadharma/tbl_pembelian_jasa/adminlte310_tbl_pembelian_cetak_pengajuan_pembayaran_print.php', $data, true);
	...
	public function pembayaran_ke_supplier()
	{
		...
		$sql = "SELECT 
		tbl_pembelian_jasa_a.id as id, 
		tbl_pembelian_jasa_a.uuid_spop as uuid_spop, 
        tbl_pembelian_jasa_a.tgl_po as tgl_po,
        tbl_pembelian_jasa_a.spop as spop,
        tbl_pembelian_jasa_a.jumlah as jumlah,
        tbl_pembelian_jasa_a.harga_satuan as harga_satuan,
		...
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/pembayaran/adminlte310_pembayaran_list_ke_supplier_jasa', $data);
	}
	...
	public function read($id)
	{
		...
		if ($row) {
			...
			$this->load->view('anekadharma/tbl_pembelian_jasa/tbl_pembelian_read', $data);
		} else {
			$this->session->set_flashdata('message', 'Record Not Found');
	...
	public function create_action()
	{
		...
		if (($this->form_validation->run() == FALSE) or is_null($this->input->post('uuid_supplier', TRUE))  or is_null($this->input->post('uuid_konsumen', TRUE))) {
			...
			// GET SPOP
			$sql = "SELECT MAX(`spop`) as maxSPOP FROM `tbl_pembelian_jasa`";
			...
			if (isset($_POST["uraian"])) {
				...
				while (list($key, $value) = each($hoby)) {
					...
					$this->_sync_persediaan_for_pembelian_jasa_id((int) $id_new_pembelian);
				}
			}
	...
	public function create_add_uraian_update($uuid_spop = null)
	{
		...
		// $this->load->view('anekadharma/tbl_pembelian_jasa/tbl_pembelian_form', $data);
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_pembelian_jasa/adminlte310_tbl_pembelian_form_spop_ready', $data);
	}
	...
	public function create_action_tambah_barang_per_spop($uuid_spop = null)
	{
		...
		$this->_sync_persediaan_for_pembelian_jasa_id((int) $inserted_id);
	...
	public function create_action_uuid_spop_update_per_id($id)
	{
		...
		$sql_data_pembelian = "SELECT * FROM `tbl_pembelian_jasa` WHERE `id`='$id'";
		...
		// Sinkronkan kembali record persediaan sesuai perubahan detail pembelian jasa.
		$this->_sync_persediaan_for_pembelian_jasa_id((int) $id);
	...
	public function create_action_uuid_spop_update($uuid_spop = null)
	{
		...
		if ($uuid_spop) {
			...
			$inserted_id = $this->Tbl_pembelian_jasa_model->insert($data); // insert untuk data lanjutan uuid_spop sudah ada
			$this->_sync_persediaan_for_pembelian_jasa_id((int) $inserted_id);
			...
			$inserted_id = $this->Tbl_pembelian_jasa_model->insert_spop($data);
			$this->_sync_persediaan_for_pembelian_jasa_id((int) $inserted_id);

			$this->db->where('id', $inserted_id);
			$GET_DATA_PEMBELIAN = $this->db->get('tbl_pembelian_jasa');
		...
		redirect(site_url('tbl_pembelian_jasa/create_add_uraian/' . $get_uuid_spop_generating));
	}
	...
	public function create_add_uraian($uuid_spop = null)
	{
		...
		$data = array(
			'data_ALL_per_SPOP' => $RESULT_per_uuid_spop,
			'button' => 'Simpan',
			'action' => site_url('tbl_pembelian_jasa/create_action_uuid_spop/' . $uuid_spop),
			'id' => set_value('id'),
			'tgl_po' => $row_per_uuid_spop->tgl_po,
			// 'nmrsj' => $row_per_uuid_spop->nmrsj,
			'nmrfakturkwitansi' => $row_per_uuid_spop->nmrfakturkwitansi,
	...
	public function create_action_uuid_spop($uuid_spop = null)
	{
		...
		if ($uuid_spop) {
			...
			$GET_ID_PEMBELIAN_AFTER_INSERT = $this->Tbl_pembelian_jasa_model->insert($data); // insert untuk data lanjutan uuid_spop sudah ada
			$this->_sync_persediaan_for_pembelian_jasa_id((int) $GET_ID_PEMBELIAN_AFTER_INSERT);
			$get_uuid_spop_generating = $uuid_spop;
			...
			$GET_ID_PEMBELIAN_AFTER_INSERT = $this->Tbl_pembelian_jasa_model->insert_spop($data);
			$this->_sync_persediaan_for_pembelian_jasa_id((int) $GET_ID_PEMBELIAN_AFTER_INSERT);
		...
		// die;
		// Pembelian jasa tidak masuk gudang/persediaan
		if (false) {
		// PERSEDIAAN
		...
		$sql_update_tbl_pembelian = "UPDATE `tbl_pembelian_jasa` SET `uuid_persediaan`='$GET_uuid_persediaan',`id_persediaan_barang`='$GET_ID_PERSEDIAAN' WHERE `id`='$GET_ID_PEMBELIAN_AFTER_INSERT'";
	...
	public function cetak_belanja_per_spop($uuid_spop = null)
	{
		...
		// 2.C. MENAMPILKAN FILE DATA
		// $data = array_merge($data);
		$html = $this->load->view('anekadharma/tbl_pembelian_jasa/adminlte310_cetak_pembelian.php', $data, true);
	...
	public function update_per_spop($uuid_spop)
	{
		...
		// $this->template->load('anekadharma/adminlte310_anekadharma', 'anekadharma/tbl_pembelian_jasa/adminlte310_tbl_pembelian_list_per_spop', $data);
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_pembelian_jasa/adminlte310_tbl_pembelian_list_per_spop', $data);
	}
	...
	public function update_per_id($uuid_pembelian)
	{
		...
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_pembelian_jasa/adminlte310_tbl_pembelian_form_per_id', $data);
	}
	...
	public function update_status_per_spop($uuid_spop = null)
	{
		...
		// SELECT `status_spop` FROM `tbl_pembelian_jasa` WHERE `uuid_spop`="53d056417ed111ef95300021ccc9061e";
		...
		// print_r($data);
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_pembelian_jasa/adminlte310_tbl_pembelian_list_per_spop_status', $data);
	}
	...
	public function delete($id)
	{
		...
		if ($row) {
			$id_persediaan_link = isset($row->id_persediaan_barang) ? (int) $row->id_persediaan_barang : 0;
			$uuid_persediaan_link = isset($row->uuid_persediaan) ? trim((string) $row->uuid_persediaan) : '';
			$this->Tbl_pembelian_model->delete($id);
			$this->_resync_or_cleanup_persediaan_link_jasa($id_persediaan_link, $uuid_persediaan_link);
			$this->session->set_flashdata('message', 'Delete Record Success');
			redirect(site_url('tbl_pembelian'));
		} else {
			$this->session->set_flashdata('message', 'Record Not Found');
	...
	public function delete_by_uuid_pembelian_from_per_spop_update($uuid_pembelian = null, $get_uuid_spop = null)
	{
		...
		$get_id_persediaan_di_tbl_pembelian = isset($get_id_pembelian->id_persediaan_barang) ? (int) $get_id_pembelian->id_persediaan_barang : 0;
		$get_uuid_persediaan_di_tbl_pembelian = isset($get_id_pembelian->uuid_persediaan) ? trim((string) $get_id_pembelian->uuid_persediaan) : '';
		...
		if ($row) {
			...
			$this->_resync_or_cleanup_persediaan_link_jasa($get_id_persediaan_di_tbl_pembelian, $get_uuid_persediaan_di_tbl_pembelian);
	...
	public function excel()
	{
		...
		if ($source !== 'tbl_pembelian_jasa') {
			show_error('Export tidak valid untuk modul pembelian jasa.', 403);
			return;
		}

		$Tbl_pembelian_jasa_rows = array();
		$ids_param = $this->input->get('ids', TRUE);
		if (!empty($ids_param)) {
			$ids = array_values(array_unique(array_filter(array_map('intval', explode(',', $ids_param)))));
			if (!empty($ids)) {
				$Tbl_pembelian_jasa_rows = $this->_get_pembelian_jasa_rows_for_excel_by_ids($ids);
			}
		}

		if (empty($Tbl_pembelian_jasa_rows)) {
			$session_ids = $this->session->userdata('filter_tbl_pembelian_jasa_ids');
			if (is_array($session_ids) && count($session_ids) > 0) {
				$Tbl_pembelian_jasa_rows = $this->_get_pembelian_jasa_rows_for_excel_by_ids($session_ids);
			}
		}

		$Get_date_awal = $this->session->userdata('filter_tbl_pembelian_jasa_date_awal');
		$Get_date_akhir = $this->session->userdata('filter_tbl_pembelian_jasa_date_akhir');
		...
		if (empty($Tbl_pembelian_jasa_rows)) {
			...
			$Tbl_pembelian_jasa_rows = $this->_get_pembelian_jasa_rows_for_excel($Get_date_awal, $Get_date_akhir);
		}

		$namaFile = 'Data_pembelian_jasa_' . date('Y-m-d_H-i-s') . '.xlsx';
		$tablehead = 0;
		$tablebody = 1;
		$nourut = 1;
		...
		foreach ($Tbl_pembelian_jasa_rows as $data) {
			$kolombody = 0;
	...
	public function pecah_satuan_proses($uuid_persediaan = null)
	{
		...
		$sql_pembelian_barang = "SELECT `uuid_persediaan`,`id_persediaan_barang`,`uuid_barang`,`uraian`,sum(`jumlah`) as jumlah_beli FROM `tbl_pembelian_jasa` WHERE `id_persediaan_barang`='$Get_id_persediaan_barang'";
		...
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_pembelian_jasa/adminlte310_tbl_pembelian_form_pecah_satuan_barang', $data);
	}
	...
	public function rollback_satuan_proses($uuid_persediaan = null)
	{
		...
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_pembelian_jasa/adminlte310_tbl_pembelian_form_rollback_barang', $data);
	}
	...
	public function jurnal_pembelian()
	{
		...
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_pembelian_jasa/adminlte310_jurnal_pembelian', $data);
	}
	...
	public function setting_kode_akun_pembelian2()
	{
		...
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_pembelian_jasa/adminlte310_tbl_pembelian_setting_kode_akun', $data);
	}
	public function jurnal_pembelian2()
	{
		...
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_pembelian_jasa/adminlte310_tbl_pembelian_list__jurnal_pembelian', $data);
	}
	...
	public function jurnal_pembelian2_cekBelumAdaKodeAkun()
	{
		...
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_pembelian_jasa/adminlte310_tbl_pembelian_list_jurnal', $data);
	}
	...
	public function input_kode_akun($uuid_spop = null)
	{
		...
		// print_r($data);
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_pembelian_jasa/adminlte310_tbl_pembelian_list_per_spop_kode_akun', $data);
	}
	...
	public function ubah_kode_akun($uuid_spop = null)
	{
		...
		$sql = "SELECT `spop`,`kode_akun`,`kode_pl`,`kode_bb` FROM `tbl_pembelian_jasa` WHERE `uuid_spop`='$uuid_spop' GROUP by `uuid_spop`,`kode_akun`";
	...
	public function kas_kecil()
	{
		...
		$this->template->load('anekadharma/adminlte310_anekadharma_topnav_aside', 'anekadharma/tbl_pembelian_jasa/adminlte310_kas_kecil', $data);
	}
