...
class Sys_supplier extends CI_Controller
{
    ...
    public function refresh_uuid_supplier()
    {

        $sql = "SELECT * FROM `data_supplier` ORDER BY nama_supplier asc";

        foreach ($this->db->query($sql)->result() as $m) {

            $data = array(
                // 'uuid_supplier' => $this->input->post('uuid_supplier',TRUE),
                'kode_supplier' => str_replace(" ","",$m->nama_supplier),
                'nama_supplier' => $m->nama_supplier,
                'nmr_kontak_supplier' => $m->nmr_kontak_supplier,
        ...
        print_r("Selesai insert data_supplier ke sys_supplier");
