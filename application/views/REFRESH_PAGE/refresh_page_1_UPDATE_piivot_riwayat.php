...
<body>
	...
	$sql_sales = "SELECT id,uuid_sales,nama_sales FROM `tbl_sales` WHERE uuid_sales = '$uuid_sales_process'";
	$data_sales = $this->db->query($sql_sales)->row();
	
	// $base_URL = base_url();	
	$get_nama_sales = $data_sales->nama_sales;
	$get_uuid_sales_selected = $uuid_sales_process;


	$data = array(
		'uuid_sales_process' => $uuid_sales_selected,
		'id_tingkat' => $id_cek_data_produk_tingkat,
		'thn_process' => $thn_process,
	...
	<script type="text/javascript">
		...
		$(function() {
			...
			$x_link =  "index.php/Trans_pesan_jual_retur_pivot/PIVOT_update_perSALES_perTingkat/";
