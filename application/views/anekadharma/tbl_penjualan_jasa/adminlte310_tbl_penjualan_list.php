<div class="content-wrapper">
    ...
    <section class="content">
        ...
        <?php
        if (!empty($tgl_awal_display)) {
            $Get_date_awal = $tgl_awal_display;
        } elseif (date("Y", strtotime($date_awal)) < 2020) {
            $Get_date_awal = date("d-m-Y");
        } else {
            $Get_date_awal = date("d-m-Y", strtotime($date_awal));
        }

        if (!empty($tgl_akhir_display)) {
            $Get_date_akhir = $tgl_akhir_display;
        } elseif (date("Y", strtotime($date_akhir)) < 2020) {
                    ...
                    <div class="card-header">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-5">
                                ...
                                $action_cari_between_date = site_url('tbl_penjualan_jasa/cari_between_date');
...
<script>
    ...
    function cetakExcelPenjualan() {
        ...
        if (!ids.length) {
            alert('Tidak ada DATA PENJUALAN JASA untuk diekspor. Periksa filter/search DataTable atau rentang tanggal.');
            return;
        }
        ...
        var source = sourceEl ? sourceEl.value : 'tbl_penjualan_jasa';
        var url = <?php echo json_encode(site_url('tbl_penjualan_jasa/excel')); ?>
...
(function() {
    ...
    function saveFilterTanggalSession(awal, akhir) {
        if (!window.sessionStorage || !awal || !akhir) {
            return;
        }
        try {
    ...
    function loadFilterTanggalSession() {
    ...
    function restoreFilterTanggalDariSession() {
        if (filterRestoreAttempted || skipFilterRestore) {
            return;
        }
        filterRestoreAttempted = true;

        var stored = loadFilterTanggalSession();
        if (!stored) {
            return;
        }
        ...
        if (tanggalInputSama(inpAwal.value, stored.tgl_awal) && tanggalInputSama(inpAkhir.value, stored.tgl_akhir)) {
            return;
        }
    ...
    function buildUrlInputPenjualanBaru() {
        var baseCreate = <?php echo json_encode(site_url('tbl_penjualan_jasa/create')); ?>;
