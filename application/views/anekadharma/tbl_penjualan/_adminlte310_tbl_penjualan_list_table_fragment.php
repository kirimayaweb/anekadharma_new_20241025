...
if (!isset($penjualan_table_id) || trim((string) $penjualan_table_id) === '') {
	$penjualan_table_id = 'tglSPOPFreeze';
}
if (!function_exists('tbl_penjualan_render_tgl_dan_badge_bayar')) {
	function tbl_penjualan_proses_bayar_key($proses_bayar)
	{
		$proses = strtolower(trim((string) $proses_bayar));
		return ($proses === 'belum_bayar' || $proses === '') ? 'belum_bayar' : 'terbayar';
	}

	function tbl_penjualan_render_tgl_dan_badge_bayar($tgl_jual, $proses_bayar)
	{
		echo date('d M Y', strtotime($tgl_jual));
		$proses = strtolower(trim((string) $proses_bayar));
		if ($proses === 'belum_bayar' || $proses === '') {
			echo '<span class="penjualan-badge-bayar penjualan-badge-bayar-p" title="Belum terbayar (proses_bayar: belum_bayar)">P</span>';
			return;
		}
...
?>
                        <table id="<?php echo htmlspecialchars($penjualan_table_id, ENT_QUOTES, 'UTF-8'); ?>" class="display nowrap penjualan-list-table" style="width:100%">
                            ...
                            <tbody>
                                ...
                                $start = 0;
                                foreach ($Tbl_penjualan_tab_data as $list_data) {
                                ...
                                ?>
                                        ...
                                        <tr class="row-penjualan-data" data-penjualan-id="<?php echo (int) $list_data->id; ?>" data-proses-bayar="<?php echo htmlspecialchars(tbl_penjualan_proses_bayar_key(isset($list_data->proses_bayar) ? $list_data->proses_bayar : ''), ENT_QUOTES, 'UTF-8'); ?>">
