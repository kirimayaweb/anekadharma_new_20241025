...
class Tbl_penjualan_jasa_model extends CI_Model
{
    ...
    const BARANG_JASA_VALUE = 'jasa';

    private function _scope_jasa()
    {
        $this->db->where($this->barang_jasa, self::BARANG_JASA_VALUE);
    }

    private function _merge_jasa_data($data)
    {
        if (!is_array($data)) {
            return $data;
        }
        $data[$this->barang_jasa] = self::BARANG_JASA_VALUE;
        return $data;
    }
    ...
    function konsumen_tagihan()
    {
        ...
        WHERE tbl_penjualan_a.barang_jasa = 'jasa'
    ...
    {
        $data = $this->_merge_jasa_data($data);
...
/* End of file Tbl_penjualan_jasa_model.php */
/* Location: ./application/models/Tbl_penjualan_jasa_model.php */
